...
legal/        法務3ページ（プライバシー/EULA/特商法。夜テーマ・style.css 共用。文面は不変）
ffkit/        ffkit の LP（夜テーマ・style.css 共用。入手節は一般配布前の正直表記）
kokohawatashiga/  ココハワタシガの LP＋法務2ページ（privacy.html / terms.html — 正本は onmt/receipt_flutter の site/。App Store 提出用 URL の実体。LP は意図的に夜テーマ非適用・後述）
...
404.html      404ページ（必須 — 無いと Pages が SPA とみなし、存在しないパスにトップを 200 で返す）
```
...
- **露出の指針は「押さないが、見つかる」**（Koji.O 方針 2026-07-17・上の半歩調整）。売り込み・煽り・
  referral の類はやらないが、**探した人に必ず届く**状態は整える＝ sitemap.xml / robots.txt / canonical /
  OGP画像（`assets/og.png`）/ JSON-LD 構造化データ。ページを増減したら sitemap と構造化データも追従させること。
  露出を増やす手段は「実質を見せる」方向のみ（実績・技術ノート等）。効果は Web Analytics の検索・参照元流入で測る。
- **外部リソースは一切読み込まない**（CSP で強制）。フォント・画像・JS すべて自前。唯一の例外はサイト解析（Cloudflare Web Analytics・Cookieなし匿名集計）。運用と読み方は [`docs/analytics.md`](docs/analytics.md)。
