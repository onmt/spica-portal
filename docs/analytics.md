...
| `kokohawatashiga/`（LP・privacy・terms） | ❌（既定で対象外） | 「JSなし」が Koji.O 判断（対象が IT に不慣れな一般ユーザー）。かつ privacy/terms の正本は `onmt/receipt_flutter`。データが欲しければ opt-in 可（下記） |

> ココハワタシガも計測したい場合: 3 ファイルの `</body>` 直前に同じビーコン 1 行を足し、
> LP のプライバシー表記を receipt_flutter 側の正本と整合させること。
