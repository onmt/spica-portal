...
| **読み取り結果は「候補」提示→ユーザーが確認・修正**（店名/金額/日付/科目/税額/支払方法/登録番号の候補・OCR完了時の実文言「情報を確認して必要に応じて修正してください」） | LP 考えかた節・つかいかた「たしかめる」 | `lib/features/editor/presentation/add_receipt_screen_ocr.dart` | ✅ | 2026-07-17 |
...
| **メイン画面（操作面）の反転は実装済み**（左右/上下・ショートカット x/z・反転軸＝マーカー線で読み位置保持・出力窓へ非伝播・localStorage永続）。**タブレットモードのある PC 等**でメイン画面をハーフミラー越しのプロンプターに使う用途（iPad/スマホ単体アプリは今後の予定・2026-07-18 Koji 修正。旧「iPad/スマホ」記述は実態と乖離）。課金線v3の Solo 解禁対象 | prompter/ 機能比較表（Solo「画面の反転」行） | `spica-prompter` PR #33（2026-07-12 14:49 マージ）`engine.js` setSelfFlip・`verify-selfflip.js` ALL PASS | ✅ | 2026-07-12 |
...
> **Mac 版（2026-07-18 Koji）**: PC と同時リリース可能な水準まで開発が進行。ただし正式リリース前のため、
> LP・タイトルに「Mac」を断定表記するのは**リリース確定後**とする（それまでは PC 中心の表現に留める）。
> 確定したらこの行を「対応: PC / Mac」へ更新し、`/prompter/` のタイトルと本文へ Mac を明記する。
> **対応プラットフォーム（現況・2026-07-18）**: PC（開発中）＋ Mac（同時リリース見込み）。iPad/スマホ単体アプリは今後。
