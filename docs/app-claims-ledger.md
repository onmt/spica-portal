...
| 主張 | サイト掲載箇所 | 正本（在り処） | 状態 | 最終確認 |
|---|---|---|---|---|
| PC 向け有料製品（MoR 経由）は**購入後30日以内の返金**に応じる（群標準。Pro LP の「30日間の無条件返金」を全 PC 製品へ展開） | legal/tokushoho.html 返品欄・prompter/ Pro カード・ffkit/ 商用カード（予定表記） | `legal/tokushoho.html`（文面の実体）＋`onmt/spica` catalog/product-lineup §3/§6（返金30日・2026-07-12 検証済）。**playbooks 側の方針正本への追従が未**（要 PR） | ✅ | 2026-07-13 |
| Solo→Pro は**差額 ¥11,800** でアップグレード可（買い直し不要） | prompter/ Solo プランカード | `onmt/spica` `billing/catalog.yaml`（teleprompter-*・2026-07-12 検証済） | ✅ | 2026-07-13 |
| **EULA §1「端末数の計数や、起動のたびのオンライン確認は行いません」・§5「収集・外部送信しません」が、ライセンス方式 0009（Solo/Pro＝90日ごとオンライン鍵更新・機器ハッシュ送信・台数認証 1/2/2）および prompter LP「2台まで認証」と乖離**（EULA 最終更新 2026-06-28 ＝ 0007–0009 改訂前のまま） | legal/eula.html §1・§5 | `onmt/spica` `decisions/0007`〜`0009`・`docs/license-protocol.md`（**本セッションでは正本未接続のため文面改訂は保留**） | ⚠️ | 2026-07-13 |

> 価格体系のレビューと改訂案（セール窓の移設・ffkit 商用価格・v2 アップグレード権など、catalog 正本側の変更を要するもの）は
> [`docs/pricing-review-2026-07-13.md`](pricing-review-2026-07-13.md) に集約。catalog 反映後にこの台帳と LP を追従させる。
